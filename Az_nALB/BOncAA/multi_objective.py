...
protein_path = '../BOncAA/MSA-nAlb-FcRn-B2M.pdb'
...
target = Pose()
target_chain = complexed.split_by_chain()[4]
...
def bayesian_optimization(objective, binding_coordinates, target_range, iterations, complexed, target,
                          ...
                          mutationrelx=True, mutation_dump=True):
    ...
    for i in range(iterations):
        ...
        acq_function = Acquisition_Function(model, beta=1.5, gamma=0.05, psi=0.05, nu=1, obj_weights=[1,1])
...
evaluated, observations = bayesian_optimization(objective, b_coordinates, target_range, 5, complexed, target, chain, ncaa, smiles, radius_relax = 7,
                      trials = 5, mutationrelx = True, mutation_dump = True)

evaluated = np.array(evaluated)
evaluated = [int(point) for point in evaluated]
observations = np.array(observations.squeeze(-1))

import pandas as pd
df = {'Points Evaluated': evaluated, 
      'Binding ddG': observations[:,0],
      'Folding ddG': observations[:,1]}

df = pd.DataFrame(df)

df.to_csv('nALBSingleRun.csv', index = False)
